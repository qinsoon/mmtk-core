...
impl SideMetadataSanity {
    ...
    fn get_all_specs(&self, global: bool) -> Vec<SideMetadataSpec> {
        ...
        std::collections::HashSet::<SideMetadataSpec>::from_iter(specs).into_iter().collect()
    }
...
mod tests {
    ...
    fn test_side_metadata_sanity_get_all_local_specs() {
        ...
        sanity.verify_metadata_context("space1", &SideMetadataContext { global: vec![], local: vec![spec_1.clone()] });
        sanity.verify_metadata_context("space2", &SideMetadataContext { global: vec![], local: vec![spec_1.clone()] });
