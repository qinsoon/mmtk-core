...
pub struct MockVM {
    ...
    scan_roots_in_mutator_thread: MockMethod<(VMWorkerThread, &'static mut Mutator<MockVM>, Box<dyn RootsWorkFactory<<MockVM as VMBinding>::VMEdge>>), ()>,
    scan_vm_specific_roots: MockMethod<(VMWorkerThread, Box<dyn RootsWorkFactory<<Self as VMBinding>::VMEdge>>), ()>,
...
impl Default for MockVM {
    fn default() -> Self {
        Self {
            ...
            scan_roots_in_mutator_thread: MockMethod::new_unimplemented(),
            scan_vm_specific_roots: MockMethod::new_unimplemented(),
...
impl VMBinding for MockVM {
    ...
    fn weakref_set_referent(reference: ObjectReference, referent: ObjectReference) {
    ...
    fn scan_roots_in_mutator_thread(
        ...
        mock!(scan_roots_in_mutator_thread(tls, mutator, Box::new(factory)))
    }
    fn scan_vm_specific_roots(tls: VMWorkerThread, factory: impl RootsWorkFactory<Self::VMEdge>) {
        mock!(scan_vm_specific_roots(tls, Box::new(factory)))
    }
