...
pub trait ImmixSpaceExt<VM: VMBinding>: Space<VM> {
    ...
    /// Hole searching: find the next range of recyclable lines starting from `search_start`.
    fn get_next_available_lines(&self, copy: bool, search_start: Line) -> Option<(Line, Line)>;
    /// Post-copy routine invoked by copy contexts after copying an object into this space.
    fn post_copy(&self, object: ObjectReference, bytes: usize);

    /// Check if current GC is a defrag GC. Used by [`ImmixHybridCopyContext`] to choose between
    /// its copy and defrag allocators.
    ...
    /// If concurrent marking is currently eagerly marking new allocations, mark the given range
    /// of lines.
    ///
    /// Default no-op: LXR's concurrent marking uses reference counts, not the line-mark table.
    /// Overridden by `ImmixSpace` (used by e.g. ConcurrentImmix).
    fn eagerly_mark_new_lines(&self, _start: Line, _end: Line) {}
}
...
impl<VM: VMBinding> ImmixSpace<VM> {
    ...
    pub fn new(
        ...
        Self::new_with_extra_side_metadata_specs(args, space_args, vec![])
    }
    ...
    pub fn new_with_extra_side_metadata_specs(
        ...
        let mut local_side_metadata_specs = Self::side_metadata_specs();
        local_side_metadata_specs
            .extend(metadata::extract_side_metadata(&extra_local_side_metadata_specs));
        let common =
            CommonSpace::new(args.into_policy_args(true, false, local_side_metadata_specs));
    ...
    pub(crate) fn prepare(
        ...
        if major_gc {
            self.reset_mark_state_to_marked();
    ...
    /// Acquire a fresh block's worth of pages from the page resource and construct the
    /// corresponding [`Block`]. This is the low-level primitive shared by the tracing and RC
    /// flavors of block acquisition; it does not initialize the block or update any bookkeeping.
    pub(crate) fn acquire_clean_block_raw(
        &self,
        tls: VMThread,
        alloc_options: AllocationOptions,
    ) -> Option<Block> {
        ...
        Some(Block::from_aligned_address(block_address))
    }
    ...
    pub fn get_clean_block(
        ...
        let block = self.acquire_clean_block_raw(tls, alloc_options)?;
        self.defrag.notify_new_clean_block(copy);
        block.init(copy, false, false);
...
impl<VM: VMBinding> ImmixSpaceExt<VM> for ImmixSpace<VM> {
    fn get_clean_block(
        &self,
        tls: VMThread,
        copy: bool,
        ...
        ImmixSpace::get_clean_block(self, tls, copy, alloc_options)
    }
    fn get_reusable_block(&self, copy: bool) -> Option<Block> {
        ImmixSpace::get_reusable_block(self, copy)
    }
    fn get_next_available_lines(&self, copy: bool, search_start: Line) -> Option<(Line, Line)> {
        ImmixSpace::get_next_available_lines(self, copy, search_start)
    }
    fn post_copy(&self, object: ObjectReference, bytes: usize) {
        ImmixSpace::post_copy(self, object, bytes)
    }
    fn prepare_new_clean_block_for_allocator(&self, block: Block) {
        // Bulk clear stale line mark state
        Line::MARK_TABLE.bzero_metadata(block.start(), Block::BYTES);
        self.eagerly_mark_new_lines(block.start_line(), block.end_line());
    }
    fn eagerly_mark_new_lines(&self, start: Line, end: Line) {
        if self.should_allocate_as_live() {
            let state = self.line_mark_state.load(Ordering::Acquire);
            Line::eager_mark_lines::<VM>(state, start..end);
        }
    }
    fn in_defrag(&self) -> bool {
        ImmixSpace::in_defrag(self)
    }
}
...
impl<VM: VMBinding> ImmixCopyContext<VM> {
    ...
    fn get_space(&self) -> &'static dyn ImmixSpaceExt<VM> {
