...
pub fn create_pp_mutator<VM: VMBinding>(
    ...
    let pp = mmtk.plan.downcast_ref::<PageProtect<VM>>().unwrap();
    let config = MutatorConfig {
        allocator_mapping: &ALLOCATOR_MAPPING,
        space_mapping: Box::new({
            let mut vec = create_space_mapping(RESERVED_ALLOCATORS, true, pp);
            vec.push((AllocatorSelector::LargeObject(0), &pp.space));
            vec
        }),
    ...
    Mutator {
        allocators: Allocators::<VM>::new(mutator_tls, pp, &config.space_mapping),
